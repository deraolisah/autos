...
const VehicleGrid = () => {
    ...
    if (error) {
        return (
            <div className='rounded-lg overflow-hidden min-h-48 text-red-500 text-center duration-300 transition-all relative p-2'>
                <div className='w-full h-full bg-light-alt/50 dark:bg-dark-alt/50 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-2 flex flex-col items-center justify-center gap-2 duration-300 transition-all'>
                    {/* <img src='' alt='' className='' /> */}
                    Error: {error}                
                    <button className='cursor-pointer border border-transparent text-dark dark:text-light bg-light dark:bg-dark hover:border-light-alt dark:hover:border-dark-alt p-1.5 px-4 shadow shadow-red-500/15 flex items-center gap-1 rounded-full duration-300 transition-all' onClick={()=> { window.location.reload()}}>
